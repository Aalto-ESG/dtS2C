"""
Experiment with Kubernetes Horizontal Pod Autoscaling (HPA) enabled.
...
from warehouse import burst_feeder, day_night_feeder
...
n_cycles = 1  # How many day-night cycles we run for each model in the given time?
max_throughput_scale = 0.95  # Better to feed data just below the max throughput to avoid congestion
deploy_worker_template_path = "kubernetes_templates/worker_template_hpa.yaml"  # Template for running the experiments
deploy_worker_experiment_path = None  # This file will be created from the template
deploy_master_template_path = "kubernetes_templates/master_template_hpa.yaml"  # Template for running the experiments
deploy_master_experiment_path = None  # This file will be created from the template
deploy_hpa_path = "kubernetes_templates/hpa.yaml"
...
def start_hpa():
    log(f"Starting HPA from {deploy_hpa_path}")
    subprocess.run(
        ["kubectl", "apply", "-f", f"{deploy_hpa_path}"])

def stop_hpa():
    log(f"Stopping HPA from {deploy_hpa_path}")
    subprocess.run(
        ["kubectl", "delete", "-f", f"{deploy_hpa_path}"])
...
def log_hpa_state():
    ...
    log(f"--- HPA Status at {time.strftime('%Y-%m-%d %H:%M:%S')} ---")
    log(result.stdout)
...
for run in runs:
    ...
    cumulative_frames_sent = 0
    log_hpa_state()
    for i in experiment_order_run_4:
        # Feed data with several different target_mbps values, while also following a day-night-cycle
        target_mbps = get_experiment_throughput_mbps(points, i)
        ...
        frames_sent = day_night_feeder.run(dataset_path=dataset_path,
                                 duration_seconds=int(seconds_per_model),
                                 n_cycles=n_cycles,
                                 target_mbps=target_mbps,
                                 num_threads=data_feeder_threads,
                                 kafka_servers=kafka_servers,
                                 msg_id_offset=cumulative_frames_sent)
